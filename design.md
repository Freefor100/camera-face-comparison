...
5. 只读取状态为 `active` 的人员和这些人员的所有样本 embedding。
...
如果旧样本没有 `quality_score`，代码按质量等级回退；仍无法取得时使用 `0.6`。新数据目录默认 `top_k=3`；旧配置缺少该字段时也回退到 3。
...
库中没有 active 人员时返回 `unknown / empty_face_library`。只有一个 active 人员时没有第二候选，因此只检查匹配阈值。
...
- `persons`：人员名称、创建时间和 `draft/active` 状态；
