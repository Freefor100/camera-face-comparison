...
当前版本已经完成主链路、质量验证、开放集规则联合标定及运行时接入；剩余工作是多帧鲁棒性扩展、完整端到端耗时分析、UI 收尾和最终演示库验收。当前代码的准确流程见 [design.md](design.md)，任务书技术要求见 [任务书要求提取](docs/任务书要求提取.md)，需求分析见 [需求分析](docs/需求分析.md)，术语见 [术语与内部命名](docs/术语与内部命名.md)，跨质量 1:N 的完整实验和参数选择见 [Phase 5B 结果](docs/phase-5b-cross-quality-results.md)，论文与标准依据见 [开放集识别与质量评估调研](docs/技术调研-开放集识别规则与质量评估.md)。
...
| 姓名/未知人员判别 | 已接入跨质量联合标定选出的 Mean Prototype + 最高分阈值 | **算法、参数和运行时代码已统一** |
...
- 同一人员可以继续追加多张样本。每个人员的全部样本先形成一个 L2 归一化 Mean Prototype，Query 再与所有身份原型做 1:N 余弦检索。
- SQLite 保存人员、embedding、质量元数据和识别日志；样本原图保存在 `data/faces/`。
- 图片与向量分别记录 SHA-256，用于发现文件缺失、误覆盖或向量 BLOB 被改写。
- 当前唯一部署规则是 `Mean Prototype + score_threshold`，`minimum_score=0.5557855`；候选分差只展示和记录，不参与接收判定。
...
- Phase 3 已完成 10,108 条正式单因素质量测量。结果证明旧启发式质量门严重过严，旧质量总分不应作为主算法的拒绝、加权或分层阈值依据；当前只记录原始指标并给出非阻断提示。
...
- [x] 把模型提取标识、原始 embedding 缓存和后续判定产物分离，质量与接收参数变化不再触发模型重提取。
...
- [x] 保存 1,901 条 Probe 与 2,755 条 Gallery 历史拒绝记录、协议哈希和代码版本，为后续发现质量预筛污染提供证据。
- [x] 将固定阈值全量结果降级为历史诊断，不据此选择最终方法。
- [x] 固化开放集识别、模板聚合、FIQA 和大规模检索的技术调研及项目简化边界。

### Phase 3：质量门与入库拒绝规则验证

- [x] 将原始质量测量与质量策略判定拆开，为后续删除无依据硬门保留无阈值证据。
...
- [x] 检查启发式质量总分的错误分桶及 Error-versus-Reject 曲线，确认其拒绝代价远大于收益。
- [x] 冻结算法结论：无证据的数值质量硬门、软加权和分层阈值不进入最终主算法；只有解码失败、无人脸、多人脸或无效 embedding 阻断。
...
- [ ] 完成多帧与 E2E 复测后，最终冻结模型、输入校验、聚合方法和判定参数。
...
- 对所有相机、肤色、年龄、光照和姿态给出统一准确率保证。当前原始质量指标只用于操作提示，不是经过大规模人群与设备标定的 FIQA 模型。
...
- 第一张能够产生唯一有效人脸 embedding 的图片保存成功后，人员立即参与识别。
...
1. 对该人员的全部单位样本向量求平均，再归一化为一个 Mean Prototype。
2. Query 与每个身份原型计算一次余弦相似度。
...
达到阈值时输出第一候选姓名，否则输出“未知人员”。无人脸、多人脸、损坏图片和无效 embedding 不进入身份打分；尺寸、模糊、亮度、对比度只生成拍摄建议，不会伪装成 Unknown 或直接拒绝。
...
├─ experiments/phase4/            # 自然 LFW 无阈值分数与历史基线
...
当前保留的是可复用的“原始 embedding → 无阈值候选分数 → Calibration 联合标定 → 冻结参数 Evaluation”链路。早期固定阈值、质量预筛和机械跨域判定脚本已经删除，历史结果仍保留在阶段文档和 `data/experiments/`。所有逐图产物均在被 Git 忽略的 `data/` 下。
...
XQLFW 官方 6,000 Pair 验证和 QMUL 原始模型覆盖压力提取：
...
当前部署结论和一次性 Evaluation 已经固定，不再用 Evaluation 反复调参。下一步只做多帧扩展、完整 E2E 耗时与 UI 收尾，随后建立最终演示库。
...
自动化测试不访问网络、个人照片、本地模型或真实摄像头。公开数据集实验由独立 CLI 和 JSON/SQLite 产物验收；最终 Demo Gallery 与现场 UI 在 Phase 6 单独验收。
