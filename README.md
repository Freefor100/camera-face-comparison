...
| 不少于 3 个身份的标准库 | 当前本地库有 7 个身份、19 张样本，并完成完整性检查 | **阶段 6 建库与真实摄像头识别已通过** |
...
- 上述“点击到结果”是实验报告中的历史指标名称，已经包含五帧采集等待、检测、身份特征提取、标准库检索、阈值判定和日志写入，因此与人工点击后的实际体验处于同一数量级。当前界面将该字段显示为“识别用时”；显示器最终呈现画面的时间无法由界面自身准确测量，本项目不再继续细分这部分差异。
...
- [x] 本地图片已登记人员识别、未知人员拒识、重启恢复和完整性检查已自动验收；动态新增和追加已在开发与真实使用中验证。
- [x] 已在用户实际主机确认模型后端和摄像头设备正常。
...
3. 查看姓名或未知人员、最高相似度、固定阈值、候选分差、判定依据和识别用时。
...
当前部署规则和一次性独立验证集结果已经固定，不再用独立验证集反复调参。阶段 5C 已完成内存矩阵检索、多帧清晰度择优和界面收尾，阶段 5D 已完成检测与身份特征拆分、采集与检测流水线和 CUDA 回放对照；阶段 6 已建立最终演示标准库，完成真实摄像头下的本人已登记识别和其他人员未知拒识，并确认模型后端和摄像头设备正常。
